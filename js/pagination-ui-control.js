class PaginationUIControl {
  constructor(paginationId, numPerPage, onShowPage) {
    ...
    if (this.navEl) {
      // add event listeners
      this.numberButtons = this.navEl.getElementsByClassName(
      ...
      [this.prevButton, this.nextButton] = this.navEl.getElementsByClassName(
  ...
  _showButtons() {
    ...
    const currPageLink =
      this.numberButtons[this.currPage].getElementsByTagName("a")[0];
    currPageLink.classList.add("usa-current");
    currPageLink.setAttribute("aria-current", "page");

    if (this.numPages > 0) {
      this.numberButtons &&
        this.numberButtons[this.numPages - 1] &&
        this.numberButtons[this.numPages - 1]
          .getElementsByTagName("a")[0]
          .setAttribute("aria-label", `last page, Page ${this.numPages}`);
    }
